...
const centEuroFormatter = new Intl.NumberFormat("fi-FI", {
  style: "currency",
  currency: "EUR",
...
export function formatEur(value: number): string {
  return wholeEuroFormatter.format(Number.isFinite(value) ? value : 0);
}

/** "12 345,67 €" — exact to the cent, for table figures. */
export function formatEurPrecise(value: number): string {
  return centEuroFormatter.format(Number.isFinite(value) ? value : 0);
}
