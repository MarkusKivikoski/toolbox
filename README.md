...
| 🍩 [Budget Visualizer](/tools/budget-visualizer) | Combine any number of **income sources** and split the result into freely added/removed spending sections, shown as an interactive **doughnut chart** — the **budgeted total** sits in the middle (green within income, red when over), each section is a colour-coded slice, and whatever's left over becomes its own muted "left to budget" slice. Hover (or tap) a slice or legend row to focus it; shows per-section percentages, allocated total, and an over-budget warning. An optional **savings goal** (current balance + target) treats the monthly leftover as your contribution and **projects how long until you hit the target** (with a date). Plus a **reset** button. |
